...
import type { ChatMessage, TimeSlot, Product, Category, Promotion, Reservation, Order, Table, ReservationSettings, Schedule, ScheduleException } from '../types';
import { MessageSender, OrderType, ReservationStatus, ExceptionType } from '../types';
// FIX: Changed 'firebase/app' to '@firebase/app' to resolve module export errors.
import { initializeApp, getApp, getApps } from '@firebase/app';
import { getFirestore, collection, getDocs, getDoc, doc } from 'firebase/firestore';

// --- SERVER-SIDE FIREBASE & GEMINI INITIALIZATION ---
...
const fetchDataFromDoc = async <T>(collectionName: string, docId: string, defaultValue: T): Promise<T> => {
    const docRef = doc(db, collectionName, docId);
    const docSnap = await getDoc(docRef);
    return docSnap.exists() ? docSnap.data() as T : defaultValue;
}

// --- HELPERS (Adapted for server-side) ---
...
const isOrderFinished = (status: string): boolean => FINISHED_STATUSES.includes(status);
...
const isBusinessOpen = (schedule: Schedule, exceptions: ScheduleException[]): boolean => {
    const now = new Date();
...
// --- PROMPT GENERATION LOGIC (from geminiService.ts) ---

const generateMenuForPrompt = (products: Product[]): string => {
...
const formatScheduleForPrompt = (schedule: Schedule): string => {
    const days = ['lunes', 'martes', 'miércoles', 'jueves', 'viernes', 'sábado', 'domingo'];
    const dayKeys = ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'];
    
    return days.map((day, index) => {
        const daySchedule = schedule[dayKeys[index]];
        if(!daySchedule) return `${day.charAt(0).toUpperCase() + day.slice(1)}: Cerrado`;
...
const generateTablesForPrompt = (tables: Table[]): string => {
...
const generateAvailabilityForPrompt = (reservations: Reservation[], orders: Order[], settings: ReservationSettings, allTables: Table[]): string => {
...
`;
    ...
    const formattedSlots = busySlots.map(slot => {
        const tableNames = reservableTables
...
// This function now takes all data as arguments
const getSystemInstruction = (
    ...
    const isOpen = isBusinessOpen(schedule, exceptions);
    const menu = generateMenuForPrompt(products);
    const scheduleStr = formatScheduleForPrompt(schedule);
    const tablesInfo = generateTablesForPrompt(tables);
    const availabilityInfo = generateAvailabilityForPrompt(reservations, orders, settings, tables);
...
2.  **Antelación Mínima para Hoy**: Para reservas en el mismo día, la hora solicitada debe ser al menos **${settings.minBookingTime} minutos** después de la hora actual. Si un cliente pide una reserva para dentro de 10 minutos y la antelación mínima es de 60, debes informarle cortésmente y sugerir el próximo horario disponible (por ejemplo, "Lo siento, necesitamos al menos ${settings.minBookingTime} minutos de antelación para preparar tu mesa. ¿Te gustaría reservar a partir de las [hora futura calculada]?").
`;

    // The rest of the logic is the same as the original getSystemInstruction...
    // I'll copy it from the original file to keep the logic identical.
    if (actionLock === 'order') {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (type === 'chat') {
        const { history, actionLock } = payload;
        
        // Fetch all data required for the prompt
        const [products, schedule, exceptions, reservations, orders, tables, settings] = await Promise.all([
            fetchDataFromCollection<Product>('Products'),
            fetchDataFromDoc<Schedule>('Schedule', 'main', {} as Schedule),
            fetchDataFromCollection<ScheduleException>('ScheduleExceptions'),
            fetchDataFromCollection<Reservation>('Reservations'),
            fetchDataFromCollection<Order>('Orders'),
            fetchDataFromCollection<Table>('Tables'),
            fetchDataFromDoc<ReservationSettings>('ReservationSettings', 'main', {} as ReservationSettings),
        ]);
        ...
        const systemInstruction = getSystemInstruction(products, schedule, exceptions, reservations, orders, tables, settings, actionLock);
